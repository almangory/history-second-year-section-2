...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!isKeyAvailable) {
      // Local fallback responses for Grade 2 Secondary history curriculum
      ...
      return res.status(200).json({ text: reply, reply: reply });
    }
    ...
    return res.status(200).json({ text: response.text, reply: response.text });
  } catch (error: any) {
